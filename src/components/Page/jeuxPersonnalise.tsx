...
function JeuxPersonnalise() {
  ...
  useEffect(() => {
    ...
    if (storedFilteredGames) {
      const parsedFilteredGames = JSON.parse(storedFilteredGames);
      setFilteredGames(parsedFilteredGames);
      setLoadedFilteredGames(parsedFilteredGames);
    }
  }, [setFilteredGames]);
      ...
      <div className="w-full max-w-screen-lg px-4 mb-12 mx-auto">
        <div className="mt-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8">
            {loadedFilteredGames.map((game) => (
              <div className="group flex flex-col items-center" key={game.id}>
